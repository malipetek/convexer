...
import { rcloneWebDavLsd } from './rclone.js';
import { formatUpdateLogs, formatUpdateStatus, getImageUpdateRuntimeDiagnostics, getUpdateStrategy, isUpdateJobActive, parseImageRollbackRef, reconcileLatestImageUpdateResult, runImageUpdateJob } from './services/imageUpdateService.js';
import {
...
async function testBackupDestination(destination: any): Promise<string> {
  ...
  if (destination_type === 'koofr' || destination_type === 'webdav') {
    ...
    const output = await rcloneWebDavLsd({ url, user, password: pass }, remote_subfolder);
    return output || 'Connected successfully';
  }
...
router.get('/version/update/logs', async (_req: Request, res: Response) =>
{
  if (getUpdateStrategy() === 'image') {
    await reconcileLatestImageUpdateResult();
    const job = getLatestUpdateJob();
    res.json(formatUpdateLogs(job));
    return;
  }
  try {
...
router.get('/version/update/logs/saved', async (_req: Request, res: Response) =>
{
  if (getUpdateStrategy() === 'image') {
    await reconcileLatestImageUpdateResult();
    const job = getLatestUpdateJob();
    ok(res, { ...formatUpdateLogs(job), exists: Boolean(job?.logs) });
    return;
  }
  try {
    const fs = await import('fs');
...
router.get('/version/rollback/status', async (_req: Request, res: Response) =>
{
  if (getUpdateStrategy() === 'image') {
    ...
    ok(res, {
      available: Boolean(job?.rollback_ref),
      commit: rollback?.appImage || job?.rollback_ref || null,
...
router.get('/admin/preflight', async (_req: Request, res: Response) =>
{
  try {
    ...
    const imageUpdate = await getImageUpdateRuntimeDiagnostics();
    const checks = {
      docker_socket: true,
